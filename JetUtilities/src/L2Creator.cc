...
L2Creator::L2Creator(CommandLine& cl) {
    ...
    ptclips         = cl.getVector<float>  ("ptclips",         "8.");
...
void L2Creator::loopOverAlgorithms(string makeCanvasVariable) {
    ...
    while ((dirKey=(TKey*)nextDir())) {
        ...
        // Set ptclip
        float ptclip = ptclips[0];
        if (ptclipcones.size() != 0 ){
            if (ptclips.size() != ptclipcones.size() ){
                if (ptclips.size() < ptclipcones.size()) cout << "ptclips have more less entries than ptclipcones, missing conesize now using default ptclip = " << ptclip <<endl;
        ...
        if(l2l3 && (alg.find("pf")!=string::npos || alg.find("puppi")!=string::npos) && l2pffit.Contains("spline"))
            writeTextFileForCurrentAlgorithm_spline(ptclip);
        else
        ...
        //
        assert(checkFormulaEvaluator(ptclip));
...
//______________________________________________________________________________
bool L2Creator::checkFormulaEvaluator(float ptclip) {
...
//______________________________________________________________________________
void L2Creator::writeTextFileForCurrentAlgorithm_spline(float ptclip) {
    TString txtfilename = outputDir + era + "_L2Relative_" + ji->getAlias() + ".txt";
    ofstream fout(txtfilename);
    fout.setf(ios::right);
    ...
    for (unsigned int ieta=0;ieta<vector_size;ieta++) {
        ...
        if(frelcor!=0) {
            ...
            for(int isection=0; isection<spline->getNSections(); isection++) {
                ...
                //    bounds.second = 6500;
                //}
                if(bounds.second < ptclip) continue;
                ...
                fout<<setw(8) <<etamin<<setw(8)<<etamax
                    ...
                    <<setw(12)<<setprecision(8)<<( (firstline && ptclip > bounds.first) ? ptclip : bounds.first)
