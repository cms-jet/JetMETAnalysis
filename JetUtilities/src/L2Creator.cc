...
void L2Creator::loopOverEtaBins() {
  ...
  while ((hrsp=hl_rsp.next_object(indices))) {
    ...
    //
    if (hrsp->GetEntries()<1) continue;

    TF1*  frsp {0};
    frsp    = (TF1*)hrsp->GetListOfFunctions()->Last();
    ...
    if(alg.find("calo")!=string::npos) {
      ...
      if( frsp!=0 && frsp->Eval(1)!=0 ) {

	// retrieve cb norm integral 
	vector<double> vintegral(3), vnorm(3), vrange(3), vmean(3), vsigma(4), vexp(2), vpolLeft(8), vpolRight(8);
	vector<int>    voption(6);

	crystalBallRes(hrsp, false, vintegral, vnorm, vrange, vmean, vsigma, vexp, vpolLeft, vpolRight, voption); // hist fit, drawfit, intg2, g3, g3, canvas(defaut)

	// integral of the 3 gauss
	...
	if(cbPar!=-1){
	  // peak
	  peak  = frsp->GetParameter(cbPar);

	  // norm Gaus
	  if(cbPar==0||cbPar==10||cbPar==13) {
	    if(frsp->GetParameter(cbPar)<0.25*sumIntG || frsp->GetParameter(cbPar)<0.01) epeak = max(7., frsp->GetParError(cbPar));
	    else epeak = max(0.5*frsp->GetParameter(cbPar), frsp->GetParError(cbPar));
	  }
	  // mean Gaus
	  ...
	  //slope exp
	    ...
	    epeak = max(5., frsp->GetParError(cbPar));
	  }
	  // range
	  else if( cbPar==7 || cbPar==8 || cbPar==18) {
	    if     (frsp->GetParameter(cbPar)<0) { peak=0; epeak = 1;}
	    if     (frsp->GetParameter(cbPar)>2) { peak=2; epeak = 1;}
	    else epeak = max(0.1, frsp->GetParError(cbPar));
	  }
	  // pol p1*x
	  else if(cbPar==6 || cbPar==17){
	    if(frsp->GetParameter(cbPar)>20) { peak=0; epeak=max(2., frsp->GetParError(cbPar));}
	    else if(frsp->GetParameter(cbPar)<-20){ peak=0; epeak=max(2., frsp->GetParError(cbPar));}
	    else epeak = max(0.01, frsp->GetParError(cbPar));
	  }
	}
	else {
	  double histPeak{hrsp->GetXaxis()->GetBinCenter(hrsp->GetMaximumBin())};		 

	  // select the closest mean to 1 only if the corresponding norm and integral is>25% of tot gauss
	  //double closestToOne{100};
	  //for (unsigned i=0; i<vmean.size(); i++) {
	  //  double dist{fabs(vmean[i]-1)};
	  //  if( dist<closestToOne && (vIntG[i]>0.25*sumIntG || vNormG[i]>0.25*sumNormG) && vmean[i]>=0 && vmean[i]<=2) { 
	  //	peak=vmean[i]; closestToOne=dist;
	  //  }
	  //}
	  //if(closestToOne==100) cout<<"NO GAUSS 1||2||3 mean selected!! using histogram peak..."<<endl;

	  // select the closest mean to the maxhist that is closer to 1, if the norm and integral is>25% of tot gauss
	  double closestToHistPeak{100};
	  for (unsigned i=0; i<vmean.size(); i++) {
	    double dist{fabs(vmean[i]-histPeak)};
	    if( dist<closestToHistPeak && (vintegral[i]>0.25*sumIntG || vnorm[i]>0.25*sumNormG) && vmean[i]>=0 && vmean[i]<=2) { 
	      peak=vmean[i]; closestToHistPeak=dist;
	    }
	  }
	  if(closestToHistPeak==100) cout<<"NO GAUSS 1||2||3 mean selected!! using histogram peak..."<<endl;

	  cout<<"\nhist peak: "<<histPeak<<endl;
	  cout<<"fit peak: " <<peak <<endl;
	  if(peak!=0) cout<<"response: " << 1/peak <<endl;
	  cout<<"\nslected mean: "<<peak<<endl;
    ...
    if(cbPar!=-1) { cout<<"cbpar!=-1"<<endl; abscor=absrsp; eabscor=eabsrsp; }
    else if (absrsp != 0) {  
      abscor  =1.0/absrsp;
      eabscor=0.01;
      //eabscor = abscor*abscor*epeak;
    }
    else cout<<"Warning abscor==0!!"<<endl;

    cout<<"abscor: "<<abscor<<endl;
    cout<<"eabscor: "<<eabscor<<endl;
    ...
    cout<<"\njetPt/RefPt: "<<jetpt<<" / "<<refpt<<endl;
    cout<<"x_jetpt / y_resp(or par): "<< jetpt<<" / "<<abscor<<endl<<endl;
    cout<<"errx_jetpt / erry_resp(or par): "<< ejetpt<<" / "<<eabscor<<endl<<endl;
	...
	if (alg.find("HLT")!=string::npos) {
      ...
      // save graph
      //
      TCanvas *canvas = new TCanvas("", "", 700, 700);
      canvas->cd();
      gPad->SetLogx();
      gabscor->Draw();

      string savePath {outputDir+"/"+gabscor->GetName()+".png"};
...
void L2Creator::writeTextFileForCurrentAlgorithm() {
  TString txtfilename = outputDir + "/" + era + "_L3Absolute_" + ji->getAlias() + ".txt";
  ...
  for (unsigned int ieta=0;ieta<vector_size;ieta++) { 
	...
	fout<<setw(16)<<frelcor->GetParameter(p); 
      }
      fout<<endl;
    }
  }
