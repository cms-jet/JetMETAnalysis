...
TH1 * getMeanHistoFromHisto(TString cname, TString title, TH2 *off_in, double & miny, double & maxy){
  ...
  for (int nb = 1 ; nb <= histo->GetXaxis()->GetNbins() ; nb++){
    ...
    if (aux->GetEntries() > 10 && aux->GetRMS()<400) {

      TF1* fr = new TF1("fr","gaus",aux->GetMean()-1.5*aux->GetRMS(),aux->GetMean()+1.5*aux->GetRMS());
      fr->SetParameter(0,aux->GetEffectiveEntries());
...
TCanvas * getGausMeanOffset(TString cname, TString ctitle, TString algo, vector<TH2*> off, bool fixedRange, vector<pair<int,int> > npvRhoNpuBins){
  ...
  TH1D* hbin = new TH1D(Form("hbin_%s",cname.Data()),Form("hbin_%s",cname.Data()), 10000, 0.,10000.);
  hbin->GetXaxis()->SetLimits(10.0,4000.0);
  hbin->GetXaxis()->SetMoreLogLabels();
  hbin->GetXaxis()->SetNoExponent();
  hbin->GetXaxis()->SetTitle("p_{T}^{ptcl} (GeV)");
  hbin->GetYaxis()->SetTitle(ctitle);
  TCanvas* c = tdrCanvas(cname.Data(),hbin,14,11,true);
  //c->GetPad(0)->SetLogx();
  c->SetLogx();
  ...
  if(!hh[0]) {
    ...
    if (fixedRange) {
      if (algo.Contains("ak4",TString::kIgnoreCase)) hh[0]->GetYaxis()->SetRangeUser(-50.0,50.0);
      else if (algo.Contains("ak8",TString::kIgnoreCase)) hh[0]->GetYaxis()->SetRangeUser(-100.0,100.0);
    }
    hh[0]->SetStats(0);
    hh[0]->Draw();
  }
  ...
  for (unsigned int j=0;j<hh.size();j++) {
    ...
    hh[j]->GetXaxis()->SetRangeUser(10.0,4000.0);
    hh[j]->GetXaxis()->SetLimits(10.0,4000.0);
  ...
  c->Update();
  if(fixedRange) {
    if (algo.Contains("ak4",TString::kIgnoreCase)) hbin->GetYaxis()->SetRangeUser(-50.0,50.0);
    else if (algo.Contains("ak8",TString::kIgnoreCase)) hbin->GetYaxis()->SetRangeUser(-100.0,100.0);
    // if(NPV_Rho == 3)
...
TCanvas * getGausMeanOffsetOverPtref(TString cname, TString ctitle, TString algo, vector<TH2*> off, bool fixedRange, vector<pair<int,int> > npvRhoNpuBins){
  ...
  if(!hh[0]) {
    ...
    if (fixedRange) hh[0]->GetYaxis()->SetRangeUser(-0.20,0.20);
    hh[0]->SetStats(0);
    hh[0]->Draw();
  }
  ...
  for (unsigned int j=0;j<hh.size();j++) {
    scanHistoBinError(hh[j],0.1);
    if(isHistoEmpty(hh[j])) {
      cout << "WARNING::getMeanOverBinCenterHistoFromHisto histogram hh[j] = " << hh[j]->GetName() << " is empty and will be skipped." << endl;
      continue;
    }
    hh[j]->GetXaxis()->SetRangeUser(10.0,4000.0);
    hh[j]->GetXaxis()->SetLimits(10.0,4000.0);
