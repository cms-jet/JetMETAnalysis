...
//   	TF2 * f4  = new TF2("f4","[0] + ([1] * x ) *(1 + [2] * log(y))",0,50,0,1800);
      ...
      TFitResultPtr res = prof->Fit("f4","0QMS");
      Int_t fitStatus = res;
      //Int_t fitStatus = (Int_t)prof->Fit("f4","0QM");
      ...
      if(fitStatus == 0) {
         par0 = f4->GetParameter(0);
         par1 = f4->GetParameter(1);
         par2 = f4->GetParameter(2);
      }
      else {
         cout << "The fit status is " << fitStatus << endl;
         par0 = 0;
         par1 = 0;
         par2 = 0;
      }
