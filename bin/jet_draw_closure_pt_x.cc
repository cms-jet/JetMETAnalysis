...
int main(int argc,char**argv)
{
  ...
  if (!flavor.IsNull()) inf = new TFile(path+"Closure_"+algo+"_"+flavor+".root");
  else
    {
      ...
      inf = new TFile(path+"Closure_"+algo+".root");
    }
  ...
  //
  // Find the mean peak of the gaussian fit and fill the final histograms
  //
  ...
  TString ofname = outputDir+"ClosureVsPt_"+algo+".root";
  if(!flavor.IsNull()) ofname = outputDir+"ClosureVsPt_"+algo+"_"+flavor+".root";
  ...
  for(int j=0;j<3;j++)
    {
      ...
      TString ss(name);
      if(!flavor.IsNull()) ss+="_"+algo+"_"+flavor;
      else ss+="_"+algo;
      can[j] = new TCanvas(ss,ss,400,400);
      if (j<2)
         gPad->SetLogx();
	  if (ss.Contains("pf"))	
