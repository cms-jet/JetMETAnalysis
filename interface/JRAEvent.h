...
// a simple class
struct JRAEvent {
   explicit JRAEvent(int i, float f, long long l):nref(i),rho(f),rho50(f),rho_hlt(f),pthat(f),weight(f),npv(l),evt(l),run(l) { }
   JRAEvent():nref(-1),rho(-1),rho50(-1),rho_hlt(-1),pthat(-1),weight(-1),npv(-1),evt(-1),run(-1) { }
   int nref;
   float rho, rho50, rho_hlt, pthat, weight;
   long long npv, evt, run;
